...
export const VerticalSwiper = forwardRef<ExposeRef, VerticalSwiperProps>(
  (
    ...
    const goToSlide = useCallback(
      ({ page, animated = true }: SlideParams) => {
    ...
    const handleTouchEnd = useCallback(
      (e: TouchEvent) => {
        ...
        const deltaY = touch.clientY - touchStartYRef.current;
        const dy =
          Math.floor(touch.clientY) - Math.floor(touchStartYRef.current);
        ...
        /** 根据滑动距离和速度决定是否切换页面 */
        const shouldSwipe =
          Math.abs(deltaY) > thresholdPixels ||
          Math.abs(velocity.current) > baseVelocity;
        if (shouldSwipe) {
    ...
    const getTransform = useCallback(() => {
      ...
      const dragTransform = isDragging
        ? (dragOffset / (containerRef.current?.clientHeight || 1)) * 100
        : 0;
    ...
    return (
      <div
        ...
        style={{ height: screenH, touchAction: "none" }}
      >
        <div
          className={cn(
          ...
          style={{
            transitionDuration:
              isTransitioning && !isDragging ? `${duration}ms` : "0ms",
