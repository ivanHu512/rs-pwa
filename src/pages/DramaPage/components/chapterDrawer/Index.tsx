...
const ChapterDrawer: React.FC<ChapterDrawerProps> = ({ swiperRef }) => {
  const {
    bookDetail,
    chapterList,
    currentChapter,
  ...
  const { t } = useI18n();
  const { chapListClickReport } = useReport();
  ...
  const handleChapterCard = useCallback(
    async (chapter: ChapterItem | BookPreLoadType) => {
      ...
      chapListClickReport({
        _action: "chap_click",
        clicked_chap_order_id: chapter?.serial_number || 0,
      });
    },
    [navigateToChapter, currentChapter?.chapter_id],
  );
  /**
   * 获取元素数据
   */
  ...
  return (
    <Drawer
      open={drawerVisible}
      wrapClassName="z-52"
      containerClassName="max-h-[80%] pb-0"
      titleClassName="absolute top-0 left-0 px-[16px] z-[9]"
      contentClassName="flex"
      onClose={close}
      onClickMask={close}
    >
      <div className="touch-pan-y flex-col overflow-y-auto pb-[16px] w-full">
        ...
        <div className="mt-[16px] text-[14px] font-[400] text-white/50">
          <CollapsibleText
            ...
            moreButton={
              <span className="cursor-pointer text-[14px] text-[#E52E2E]">
                {t("video.more")}
              </span>
            }
          />
