...
const ControlBar: React.FC<IProps> = ({ showPlayType, serialNumber = 0, className, children, onVolume }) => {
  ...
  const {
    ...
    controlStatus,
    updateDrawerVisible,
  } = useDramaStore(useShallow(state => ({
      ...
      >
        ...
        >
          <div className="flex items-center">
            ...
            <label className="text-white/90">{t("video.ep", { num: currentSerialNumber })}</label>
            <label className="text-white/50">/{t("video.ep", { num: totalChapterNum })}</label>
          </div>
          {/* 信息图标 */}
          <i
            className={cn(
