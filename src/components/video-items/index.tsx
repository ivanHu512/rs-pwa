...
const PayModal = dynamic(() => import('@/components/pay-modal'), {
  ssr: false,
})
const AppModal = dynamic(() => import('@/components/app-modal'), {
  ssr: false,
})
const DramaPopup = dynamic(() => import('@/components/drama-popup'), {
  ssr: false,
})
const UnlockedToast = dynamic(
  () => import('@/components/video-items/unlocked-toast'),
  {
    ssr: false,
  }
)
const UnpublishModal = dynamic(() => import('@/components/unpublish-modal'), {
  ssr: false,
})
...
const VideoItems: React.FC<DramaProps> = (props) => {
  ...
  const slideChangeTimerRef = useRef<NodeJS.Timeout | null>(null)
  ...
  const handleEnded = useCallback(
    (id: string) => {
      ...
      if (
        ...
        chapter && navigateToChapter(chapter, true)
      }
    },
    [
      updateAppModalVisible,
